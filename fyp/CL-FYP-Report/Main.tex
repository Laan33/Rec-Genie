\documentclass{report}

\usepackage[english]{babel}

% Set page size and margins
% Replace `letterpaper' with `a4paper' for UK/EU standard size
\usepackage[a4paper,top=2cm,bottom=2cm,left=3cm,right=3cm,marginparwidth=1.75cm]{geometry}

% Useful packages
\usepackage{amssymb}
\usepackage{siunitx}
\PassOptionsToPackage{hyphens}{url}\usepackage{hyperref}
\hypersetup{
  colorlinks=false,
  linkcolor=blue,
  filecolor=magenta,      
  urlcolor=cyan,
  citecolor=blue,
  pdfborder={0 0 0}
}
\usepackage{cleveref}
\usepackage[utf8]{inputenc}
\usepackage[right]{lineno}
\usepackage{csquotes}
\usepackage{booktabs}
\usepackage{longtable}
\usepackage{adjustbox}
\usepackage{array}
\usepackage{url}
\usepackage{comment}
\usepackage{titlesec}
\usepackage{graphicx}
%\usepackage[compatibility=false]{caption}
\usepackage{authblk}
\usepackage{xcolor} % Load the xcolor package for color options
\renewcommand{\thetable}{\Roman{table}}
\usepackage{ulem} % Strike through text and 

\usepackage{float} % Required for [H] option in figures and tables

% Define a new format for \subsection
\titleformat{\subsection}
  {\mdseries\itshape\large} % Medium series, italic shape, and large font size
  {\thesubsection}{1em}{} % Numbering, spacing, and the section title itself


% Emerald Harvard Citation Style

% \usepackage[english]{babel}
% \usepackage[style=authoryear,backend=biber,natbib=true,maxcitenames=2,uniquelist=false]{biblatex}

\usepackage[backend=bibtex,style=numeric]{biblatex}
\addbibresource{Bibliography.bib}

% Customizing DOI field format to lowercase "doi"
%\DeclareFieldFormat{doi}{\bibstring{doi}\addcolon\space\url{#1}}

% Customizing URL field format to "available at:"
\DeclareFieldFormat{url}{\bibstring{available at}\addcolon\space\url{#1}}
\DeclareFieldFormat{urldate}{\mkbibparens{accessed \addspace#1}}

% Customizing urldate to match the required format
\DeclareFieldFormat{urldate}{%
  \mkbibparens{accessed\space%
    \thefield{urlday}\addspace%
    \mkbibmonth{\thefield{urlmonth}}\addspace%
    \thefield{urlyear}}}


% Configure cleveref
\crefformat{figure}{#2Figure~#1#3}
\Crefformat{figure}{#2Figure~#1#3}
\crefformat{table}{#2Table~#1#3}
\Crefformat{table}{#2Table~#1#3}
\crefformat{section}{#2Section~#1#3}
\Crefformat{section}{#2Section~#1#3}


% ------------------------------

% University Logo
% /Figures/University_Of_Galway_Logo__Positive_Landscape.png.png



%Front Matter
\author{\Large Cathal Lawlor\\\Large Supervisor: Dr. Colm O'Riordan}

\affil{\Large School of Computer Science, University of Galway, Ireland}

\title{Improving Conversational Recommendation Systems with Large Language Models}

\begin{document}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.8\textwidth]{Figures/University_Of_Galway_Logo__Positive_Landscape.png}
  \par\vspace{-6.0cm}
\end{figure}

\maketitle


\begin{comment}
- Chapter 1  Introduction
  Gentle introduction to the project – the field, the problem you are solving etc.
  Open questions to be tackled
  Your aims/goals.
  Contributions
  Lay out of report

- Chapter 2 Related Work/Literature
  Introduction 
  Recommender Systems – traditional approaches – collaborative filtering
  Limitations/Issues
  LLMs – overview background
  Use of LLMs in related domains – conversational IR, explanations etc.

- Chapter 3  Design
  Introduction – overview of the system 
  System architecture – include a diagram and discuss the main components in turn. (choice of approaches here)
  Discussion of design decisions
  Discussion of the data being used

- Chapter 4 Implementation
  Technologies and Languages used
  Issues when implementing; discuss challenges and solutions

- Chapter 5 Results and Evaluation   (we have some choices here)
  Illustration the system – few sample run throughs with sample queries and recommendations -tying back to the design and the various components. 
  This will show the reader how the system works.

  Evaluation

  Talk about the classic metrics ( can cover in Chap 2) and they limitations in complex system. 
  User -evaluation – tasks for users to undertake. Questions asked – we need to design these; Objective measure of the recommendations; Subjective feedback.

  Efficiency issues

- Chapter 6 Conclusion
  Summary  - re-iterate what the project was
  Conclusion – summarise main findings
  Reflections/Discussion
  Future Work


\end{comment}


%add 6 keywords
% \textbf{Keywords:} keyword 1; keyword 2; keyword 3, keyword 4; keyword 5; keyword 6


% \linenumbers
\tableofcontents

\chapter{Introduction}
\label{chapter:introduction}
\input{chapters/introduction.tex}

% ROUGH WORK FROM HERE ON
\chapter{Rough Work From Here On}
\label{chapter:roughWorkFromHereOn}

\chapter{Related Work}
\label{chapter:relatedWork}
\input{chapters/relatedWork.tex}

\chapter{Design}
\label{chapter:design}
\input{chapters/design.tex}

\chapter{Implementation}
\label{chapter:implementation}
\input{chapters/implementation.tex}

\chapter{Evaluation}
\label{chapter:evaluation}
\input{chapters/evaluation.tex}

\chapter{Results}
\label{chapter:results}
\input{chapters/results.tex}

\chapter{Conclusion}
\label{chapter:conclusion}
\input{chapters/conclusion.tex}

\appendix
\chapter{Appendix}
\label{chapter:appendix}
\input{chapters/appendix.tex}




%\break
\printbibliography

\end{document}



\begin{comment}
Evaluation 


User eval
- is it cumbersome
- does it add to the experience 
- do you feel more in control
- 
- would you use it again


Rec system eval
- does it function
- does it do it's job in a reasonable time frame
- cold start?

(Basically are they making sense, and so will the rest of the stuff be able to be evaluated)

  Do the recs seem good - do they make sense?

  Does the rec system outputs make sense?
  Do they seem good / logical
  - for that just show example and justify it
  


LLM eval
- can it handle cold start well
- can it handle inputs that are out of bounds
- 

User adjustments eval
- are the recommendation score percentages augmented 
- reload the user profile off of one of the csv






TODO FROM ALL THIS
load ratings from augmented to demo

Where's the space what's the 

Record a demo video of the system in action




\end{comment}